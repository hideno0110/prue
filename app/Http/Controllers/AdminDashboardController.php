...
class AdminDashboardController extends Controller
{
    ...
    public function index(Inventory $inventory, Shop $shop, ShopList $shop_list, RssNews $rss_news) {
        ...
        $this_month = date("m月");
        //仕入れ総数
        $inv_count = number_format($inventory->inv_count());
        //店舗数(支店含む)
        $shops = $shop->shop_count();
        //店舗数
        $shop_lists = $shop_list->shop_list_count();
        //今月の仕入れ数 
        $inv_month_num = $inventory->inv_month_num($this_year_month);
        //今月の仕入れ金額
        $inv_month_money = number_format($inventory->inv_month_money($this_year_month));
        //仕入れ予想利益
        $inv_expect_profit = $inventory->inv_expect_profit($this_year_month);
        //月別の仕入れ金額
        $monthly_purchase =  $inventory->monthly_purchase_price();
        //月別の仕入れ回数
